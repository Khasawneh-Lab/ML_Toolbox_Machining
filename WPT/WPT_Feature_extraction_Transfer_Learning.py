...
for i in range (0,numberofcase1):
    ...
    nameofdata1 = os.path.join(folderToLoad1,'WPT_Level1_Recon_%s.mat' %(namets1[i]))
...
#    name1 =  'chatter_training%d' %(i+1)
#    nameofdata1 = os.path.join(folderToLoad2,'WPT_Recon_Level1_%s.mat' %(namets2[i]))
#    exec("%s = sio.loadmat(nameofdata1)" % (name1))
#    exec('%s = %s["recon"]' %(name1,name1)) 
...
#for i in range (0,numberofcase2):
#    name2 =  'chatter_test%d' %(i+1)
#    nameofdata2 = os.path.join(folderToLoad2,'WPT_Recon_%s.mat' %(namets2[i]))
#    exec("%s = sio.loadmat(nameofdata2)" % (name2))
#    exec('%s = %s["recon"]' %(name2,name2))
...
for i in range (0,numberofcase4):
    name2 =  'chatter_test%d' %(i+1)
    nameofdata2 = os.path.join(folderToLoad4,'WPT_Recon_Level1_%s.mat' %(namets4[i]))
    exec("%s = sio.loadmat(nameofdata2)" % (name2))
    exec('%s = %s["recon"]' %(name2,name2))   
...
featuremat_training = np.zeros((numberofcase1,10))
for i in range (0,numberofcase1):
    name =  'chatter_training%d' %(i+1)
    exec('ts1=%s' %name)
    featuremat_training[i,0] = np.average(ts1)
    featuremat_training[i,1] = np.std(ts1)
    featuremat_training[i,2] = np.sqrt(np.mean(ts1**2))   
    featuremat_training[i,3] = max(abs(ts1))
    featuremat_training[i,4] = skew(ts1)
    L=len(ts1)
    featuremat_training[i,5] = sum(np.power(ts1-featuremat_training[i,0],4)) / ((L-1)*np.power(featuremat_training[i,1],4))
    featuremat_training[i,6] = featuremat_training[i,3]/featuremat_training[i,2]
    featuremat_training[i,7] = featuremat_training[i,3]/np.power((np.average(np.sqrt(abs(ts1)))),2)
    featuremat_training[i,8] = featuremat_training[i,2]/(np.average((abs(ts1))))
    featuremat_training[i,9] = featuremat_training[i,3]/(np.average((abs(ts1))))

#compute features for test set
    
featuremat_test = np.zeros((numberofcase4,10))
for i in range (0,numberofcase4):
    name =  'chatter_test%d' %(i+1)
    exec('ts2=%s' %name)
    featuremat_test[i,0] = np.average(ts2)
    featuremat_test[i,1] = np.std(ts2)
    featuremat_test[i,2] = np.sqrt(np.mean(ts2**2))   
    featuremat_test[i,3] = max(abs(ts2))
    featuremat_test[i,4] = skew(ts2)
    L=len(ts2)
    featuremat_test[i,5] = sum(np.power(ts2-featuremat_test[i,0],4)) / ((L-1)*np.power(featuremat_test[i,1],4))
    featuremat_test[i,6] = featuremat_test[i,3]/featuremat_test[i,2]
    featuremat_test[i,7] = featuremat_test[i,3]/np.power((np.average(np.sqrt(abs(ts2)))),2)
    featuremat_test[i,8] = featuremat_test[i,2]/(np.average((abs(ts2))))
    featuremat_test[i,9] = featuremat_test[i,3]/(np.average((abs(ts2))))    
...
#2 inch case:
#freq_feature_data_name_training_set = 'Freq_Features_2inch'
freq_feature_data_name_training_set = 'Freq_Features_2inch_WPT_Level1'

#2.5 inch case:
#freq_feature_data_name_training_set = 'Freq_Features_2.5inch'
#freq_feature_data_name_training_set = 'Freq_Features_2.5inch_WPT_Level1.mat'

#3.5 inch case:
#freq_feature_data_name_training_set = 'Freq_Features_3.5inch'
#freq_feature_data_name_training_set = 'Freq_Features_3.5inch_Level3'

#4.5 inch case:
#freq_feature_data_name_training_set = 'Freq_Features_4.5inch'
#freq_feature_data_name_training_set = 'Freq_Features_4.5inch_Level1'


#Features for test set
    
#2 inch case:
#freq_feature_data_name_test_set = 'Freq_Features_2inch'
#freq_feature_data_name_test_set = 'Freq_Features_2inch_WPT_Level1'

#2.5 inch case:
#freq_feature_data_name_test_set = 'Freq_Features_2.5inch'
#freq_feature_data_name_test_set = 'Freq_Features_2.5inch_WPT_Level1.mat'

#3.5 inch case:
#freq_feature_data_name_test_set = 'Freq_Features_3.5inch'
#freq_feature_data_name_test_set = 'Freq_Features_3.5inch_Level1'

#4.5 inch case:
#freq_feature_data_name_test_set = 'Freq_Features_4.5inch'
freq_feature_data_name_test_set = 'Freq_Features_4.5inch_Level1'   


freq_features_training_set = sio.loadmat(os.path.join(folderToLoad1,freq_feature_data_name_training_set))
freq_features_training_set = freq_features_training_set['Freq_Features']

freq_features_test_set = sio.loadmat(os.path.join(folderToLoad4,freq_feature_data_name_test_set))
freq_features_test_set = freq_features_test_set['Freq_Features']

#concatanate the frequency and time domain features 
featuremat_training = np.concatenate((featuremat_training, freq_features_training_set),axis = 1)
featuremat_test = np.concatenate((featuremat_test, freq_features_test_set),axis = 1)
...
for o in range(0,10):
    ...
    F_Training_Train,F_Training_Test,Label_Training_Train,Label_Training_Test= train_test_split(featuremat_training,label1, test_size=0.33)
    F_Test_Train,F_Test_Test,Label_Test_Train,Label_Test_Test= train_test_split(featuremat_test,label4, test_size=0.70)
